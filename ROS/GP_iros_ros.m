...
% Start ROS comms.
%rosinit
pose_pub = rospublisher('/firefly/command/pose', rostype.geometry_msgs_PoseStamped);
...
% Generate grid map.
grid_map.m = 0.5*ones(dim_y*map_parameters.resolution, ...
    dim_x*map_parameters.resolution);
...
while (true)
    ...
    for i = 1:size(points_meas,1)
        ...
        grid_map = take_measurement_at_point_ros(point_odometry, pcl, grid_map, ...
