...
% Gets grid co-ordinates of observed submap of the grid map.

pos_grid = env_to_grid_coordinates(pos_env, map_parameters);

submap_coordinates = [];

xl = pos_grid(1) - floor(submap_edge_size.x/2);
while (xl < 1)
    xl = xl + 1;
end
...
while (xr > map_parameters.env_dim_x/map_parameters.resolution)
    xr = xr - 1;
end
yu = pos_grid(2) + floor(submap_edge_size.y/2);
while (yu > map_parameters.env_dim_y/map_parameters.resolution)
    yu = yu - 1;
end
